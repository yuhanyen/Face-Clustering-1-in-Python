...
for key in face_dict.keys():
    ...
    for index, (image, shape, face) in enumerate(face_dict[key]):
        file_path = os.path.join(file_dir, 'face_' + str(index))
        print (file_path)
        print (shape)
        #dlib.save_face_chip(image, shape, file_path, size=150, padding=0.25)
        imCrop = image[face.top():face.bottom(),face.left():face.right(),::-1]
        cv2.imwrite(file_path+".jpg",imCrop)